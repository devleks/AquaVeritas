...
export const metadata = {
  ...
  description:
    "Twenty freshwater bodies under continuous monitoring. Lake Chad, the Aral Sea, Tonle Sap, sixteen more. Click any site for context and one-click inference.",
};

// Group sites by category for the index section
      ...
      <section className="border-b border-[color:var(--color-rule)] bg-[color:var(--color-surface-alt)]">
        <div className="mx-auto max-w-6xl px-6 py-12">
          ...
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-[color:var(--color-ink-muted)]">
            Five chronic-shrinkage anchors. Seven flooding regimes. Eight mixed-use
            sites where agriculture, settlement, and water-body dynamics interact.
            Click any dot for context and run inference on its imagery.
          </p>
        </div>
      </section>

      {/* ── Globe (75vh, the star of the page) ──────────────────────────────── */}
      <div className="relative h-[75vh] border-b border-[color:var(--color-rule)]">
        <Globe />
      </div>
            ...
            {SITES_BY_CATEGORY.map(({ cat, sites }) => (
              <div key={cat}>
                ...
                <ul className="mt-6 divide-y divide-[color:var(--color-rule)] border-t border-[color:var(--color-rule)]">
                  {sites.map((s) => (
                    <li key={s.id} className="grid grid-cols-1 gap-x-6 gap-y-1 py-5 md:grid-cols-12">
